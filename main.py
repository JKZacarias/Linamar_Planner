...
class Application(ttkb.Window):
    def __init__(self, *args, themename=None, iconphoto=None, **kwargs):
        ...
        pyglet.font.add_file(resource_path("Fonts\\Inter-VariableFont_opsz,wght.ttf"))
    ...
    def show_appointment_frame(self):
        ...
        self.appointment_frame.receive_customer_names()
...
class NewAppointment(ttkb.Frame):
    def __init__(self, parent, *args):
        ...
        self.cal_entry_var = tk.StringVar()
        self.saved_date_var = tk.StringVar()
        ...
        self.weight_deliberation = False
        ...
        self.add_btn = ttkb.Button(self.bottom_left_Frame, text="Add", bootstyle="info", state="disabled", command=self.email_check)
        ...
        self.submit_btn = ttkb.Button(self.bottom_button_frame, text="Submit", bootstyle="success", command=lambda: self.submit_order(self.dict_for_submit, self.cust_for_submit))
        ...
        self.appointment_calander.entry.configure(state="readonly", textvariable=self.cal_entry_var)

        self.error_label = ttkb.Label(self.top_left_frame, justify="center", textvariable=self.error_var, foreground="#FF0000", font=('Inter', 13))
        ...
        self.weight_var.trace('w', self.weight_check)
        self.cal_entry_var.trace('w', self.read_only_cal)

        self.appointment_calander.button.bind('<Button-1>', self.on_date_selected)

    #NewAppointment Setter Funcs
    ...
    #Main Funcs
    def read_only_cal(self, *args):
        date = self.appointment_calander.entry.get()
        if self.appointment_calander.entry.get():
            if self.saved_date_var.get() != date:
                self.saved_date_var.set(date)
                self.saved_date_var.set(date)
                self.full_check_for_add()
                self.appointment_calander.entry.configure(state="readonly")

    def on_date_selected(self, *args):
        self.appointment_calander.entry.configure(state="active")
        
    def weight_check(self, *args):
        if self.weight_var.get():
            for i in self.weight_var.get():
                if i:
                    if i.isalpha():
                        self.weight_deliberation = False
                        self.error_var.set('ERROR: Weight cannot contain letters')
                        break
                    elif not i.isnumeric():
                        self.weight_deliberation = False
                        self.error_var.set('ERROR: Weight cannot contain symbols')
                        break
                    else:
                        self.error_var.set('')
                        self.weight_deliberation = True
        else:
            self.error_var.set('')
            self.weight_deliberation = False
        self.full_check_for_add()

    def full_check_for_add(self):
        if self.customer_var.get():
            if self.item_var.get():
                if self.weight_deliberation == True:
                    if self.appointment_calander.entry.get():
    ...
    def receive_customer_names(self):
    ...
    def reset_frame(self):
        ...
        if check_file:
            ...
            self.add_btn.configure(state="disabled")
            self.appointment_calander.entry.configure(state="active")
            self.appointment_calander.entry.delete(0, tk.END)
            self.appointment_calander.entry.configure(state="readonly")

            self.weight_var.set('')
            self.cal_entry_var.set('')
            self.saved_date_var.set('')
            self.weight_deliberation = False
        else:
    ...
    def submit_order(self, the_dict, customer_param):
    ...
    def no_access_token_yet(self, application_id, client_secret, scopes):
        ...
        self.cancel_auth_btn = ttkb.Button(self.auth_code_window, text="Cancel", bootstyle="danger", command=self.cancel_auth)
        ...
        for item, item_values in the_item_dict.items():
            ...
            try:
                ...
                def construct_event_detail(event_name, **event_details):
                    request_body = {
                        'subject': f'{event_name.capitalize()}-{item_name}-{weight}-({trucks})-Ship_ID:{ship_id}',
                        'isAllDay': True
                    }
                ...
                end_day_obj = start_day_obj + datetime.timedelta(days=1)
...
class CheckStatus(ttkb.Frame):
    ...
    def checkbuttons_check(self):
        self.error_var.set('')
        #If Only customer search is selected
        if self.customer_search_var.get() == 1 and self.item_search_var.get() == 0:
            ...
            #If neither customer search or item search are selected
            self.error_var.set('')
            self.customer_label.grid_remove()
            self.customer_combo.grid_remove()
            self.item_label.grid_remove()
